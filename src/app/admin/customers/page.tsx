...
export default async function CustomersPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeading title="Customers" sub={`${customers.length} portal accounts`} actions={<OnboardCustomer demo={isDemo} />} />
      <Card className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse min-w-[720px]">
            ...
            <tbody>
              {customers.map((c) => {
                const mine = orders.filter((o) => o.customer.id === c.id && o.status !== "rejected");
                return (
                  <tr key={c.id} className="border-b border-border-soft hover:bg-surface-soft">
                    <td className="td font-medium">{c.company_name ?? "—"}</td>
                    <td className="td font-mono text-xs text-slate">{c.email}</td>
                    <td className="td text-[12.5px] text-slate-strong">{shortDate(c.created_at)}</td>

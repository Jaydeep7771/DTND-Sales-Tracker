  ...
  return (
    <>
      <header className="bg-surface border-b border-border sticky z-20" style={{ top: topOffset }}>
        <div className="max-w-[1280px] mx-auto px-4 sm:px-5 py-3 sm:py-3.5 flex items-center gap-3 sm:gap-[22px] flex-wrap">
          <div className="flex items-center gap-2.5 min-w-0 flex-[1_1_220px]">
            ...
            <div className="leading-[1.15] min-w-0">
              <div className="text-[14.5px] font-semibold tracking-[-.01em] truncate">Dynamic Traders &amp; Distributors</div>
          ...
          <div className="flex items-center gap-3 ml-auto shrink-0">
            <div className="text-right leading-[1.3] min-w-0 hidden sm:block">
              <div className="text-[12.5px] font-semibold whitespace-nowrap">{company}</div>
              <div className="text-[11px] text-slate whitespace-nowrap">{account} · Net 30{!demo && <> · <button type="button" onClick={() => signOut().then(() => router.push("/login"))} className="bg-transparent border-0 p-0 text-[11px] text-slate cursor-pointer hover:text-danger">Sign out</button></>}</div>
            </div>
            <Button onClick={() => cart.setOpen(true)} aria-label={`Open cart, ${cart.lines.length} items`}>
              Cart
      ...
      {cart.open && (
        <div className="fixed inset-0 z-[80] flex justify-end bg-[rgba(15,27,43,.45)]" onClick={() => cart.setOpen(false)}>
          <div role="dialog" aria-modal="true" aria-label="Your cart" className="w-full sm:max-w-[420px] bg-surface flex flex-col shadow-[-20px_0_50px_rgba(15,27,43,.25)]" onClick={(e) => e.stopPropagation()}>
            <div className="px-[18px] py-4 border-b border-border flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-slate mt-0.5">{cart.lines.length} line{cart.lines.length === 1 ? "" : "s"} · {num(units)} units · prices locked at submit</div>
              </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <div className="flex items-center border border-border rounded-[6px] overflow-hidden">
                        ...
                        <input value={l.quantity} aria-label="Quantity" inputMode="numeric" onChange={(e) => cart.setQty(l.product_id, Number(e.target.value) || 1)} className="w-[38px] border-0 text-center font-mono text-[12px] outline-none" />
                        <button type="button" aria-label="Increase" onClick={() => cart.setQty(l.product_id, l.quantity + 1)} className="border-0 bg-surface-soft w-7 h-7 cursor-pointer text-sm">+</button>
                      </div>
